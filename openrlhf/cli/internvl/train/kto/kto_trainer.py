...
class MultiModalKTOTrainer(Trainer):
    r"""
    ...
    def get_batch_loss_metrics(
        ...
        metrics = {}
        # [v.requires_grad for k, v in concatenated_batch.items()]
        #   [v.requires_grad_() if k not in ['label'] else v for k, v in batch.items()]
        # with torch.set_grad_enabled(True):
        batch = {k: (v.to(self.accelerator.device) if isinstance(v, torch.Tensor) else v) for k, v in batch.items()}
        batch['label'] = batch['label'].detach().tolist()
        # import pdb;pdb.set_trace()
