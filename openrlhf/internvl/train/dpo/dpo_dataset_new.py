...
class DPO_Dataset(Dataset):
    ...
    def __init__(self, template_name, meta, tokenizer, tcs_loader, num_image_token, ds_name,
                 ...
                 max_dynamic_images = 6, max_multi_image_dynamic_patch = 6, max_patches = 42, precompute_ref_log_probs=False, reference_model=None):
        super(DPO_Dataset, self).__init__()
        ...
        if 'ref_logps' in meta:
            self.file = open(meta['ref_logps'], 'rb')
        ...
        # self.reference_chosen_logps = None
        # self.reference_rejected_logps = None
    ...
    def add_column(self, name, column_data):
        assert len(self.raw_data) == len(column_data), f'新列的长度必须与数据集长度匹配{name}'
        self.additional_columns[name] = column_data
        
        # self.reference_chosen_logps = column_data[0]
        # self.reference_rejected_logps = column_data[1]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while load_try:
            ...
            try:
                ...
                # assert self.reference_chosen_logps and self.reference_rejected_logps
                # for k, v in self.additional_columns.items():
                #     ret[k] = v[i]
                if 'reference_chosen_logps' in data_item:
                    ret['reference_chosen_logps'] = torch.tensor(data_item['reference_chosen_logps'])
                if 'reference_rejected_logps' in data_item:
                    ret['reference_rejected_logps'] = torch.tensor(data_item['reference_rejected_logps'])
        ...
        for s in ['chosen', 'rejected']:
            if ret['image_flags'].sum() * self.num_image_token != (ret[f'{s}_input_ids'] == self.img_context_token_id).sum():
...
def build_dpo_datasets(data_args, tokenizer, tcs_loader, model, group_by_length=False,
                   dynamic_image_size=False, use_thumbnail=False, min_dynamic_patch=1,
                   max_dynamic_patch=6, normalize_type='imagenet', precompute_ref_log_probs=True):
    datasets = []
    ...
    for ds_name in ds_collections.keys():
        ...
        #         fix_seed=fix_seed
        #     )
        # dataset = DatasetRef(dataset=dataset, model=model, label_pad_token_id=-100, padding_value=tokenizer.pad_token_id)
...
class CustomConcatDataset(ConcatDataset):
    ...
    def add_column(self, name, column):
        # 检查新列长度是否与ConcatDataset的总长度匹配
        if len(column) != len(self):
            raise ValueError("新列的长度必须与合并后的数据集长度必须匹配")

        # 对每个子数据集进行处理，将对应部分的列数据添加到子数据集中
        offset = 0
        for dataset in self.datasets:
            dataset_length = len(dataset)
            dataset.add_column(name, column[offset:offset + dataset_length])
