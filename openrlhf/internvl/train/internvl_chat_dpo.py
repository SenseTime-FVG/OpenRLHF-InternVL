...
from dpo.dpo_dataset_new import build_dpo_datasets
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        #     model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config).eval()
    else:
        ...
        # ref_model = InternVLChatModel(internvl_chat_config, vision_model, llm).eval()
    ...
    # data_module = build_dpo_dataset(tokenizer, data_args=data_args, reference_model=deepcopy(model).cuda())
    # train_dataset = DPO_Dataset(data_dir=data_args.data_dir)
    # train_dataset = get_dpo_dataset(data_dir=data_args.data_dir)
    ...
    if training_args.task == 'dpo':
        ...
        train_dataset = build_kto_datasets(
            data_args, tokenizer, tcs_loader, model, group_by_length=training_args.group_by_length,
            dynamic_image_size=data_args.dynamic_image_size, use_thumbnail=data_args.use_thumbnail,
            min_dynamic_patch=data_args.min_dynamic_patch, max_dynamic_patch=data_args.max_dynamic_patch,
            normalize_type=data_args.normalize_type)
    ...
    #     logging_steps=10,
    # )
    # DPOTrainerSelf, MultimodalDPOTrainer
    ...
    if training_args.task == 'dpo':
        ...
        trainer = MultimodalDPOTrainer(
            model,
            ref_model=deepcopy(model), # not needed when using peft
            args=training_args,
        ...
        trainer = MultiModalKTOTrainer(
            ...
            tokenizer=tokenizer,
            # dataset_num_proc=8,
            data_collator=kto_concat_pad_data_collator
        )
    else:
        raise f'not implement {training_args.task}'
